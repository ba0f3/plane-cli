...
func TestBuildWorkspaceDigest(t *testing.T) {
	...
	records := []Record{
		makeRecord("design", "DES", 1, "Overdue blocker", issueOptions{
			state:      "In Progress",
			group:      "started",
			priority:   "high",
		...
		makeRecord("design", "DES", 2, "Due today", issueOptions{
			state:      "Todo",
			group:      "unstarted",
			targetDate: "2026-09-26",
			updatedAt:  now.Add(-time.Hour),
		}),
		makeRecord("design", "DES", 3, "Completed", issueOptions{
			state:       "Done",
			group:       "completed",
			updatedAt:   completedAt,
			completedAt: &completedAt,
		}),
		makeRecord("engineering", "ENG", 4, "Other workspace", issueOptions{
			state:     "Todo",
			group:     "unstarted",
			updatedAt: now,
		}),
...
func TestBuildUserDigestMatchesEmail(t *testing.T) {
	now := time.Date(2026, 9, 26, 9, 0, 0, 0, time.UTC)
	records := []Record{
		makeRecord("design", "DES", 1, "Alice task", issueOptions{
			state:     "Todo",
			group:     "unstarted",
			updatedAt: now,
			assignees: []plane.FlexibleUser{{ID: "u1", Email: "alice@example.com", DisplayName: "Alice"}},
		}),
		makeRecord("design", "DES", 2, "Bob task", issueOptions{
			state:     "Todo",
			group:     "unstarted",
			updatedAt: now,
			assignees: []plane.FlexibleUser{{ID: "u2", Email: "bob@example.com", DisplayName: "Bob"}},
		}),
	}
...
func makeRecord(workspace, project string, sequence int, title string, opts issueOptions) Record {
	...
	return Record{
		...
		Issue: plane.Issue{
			...
			SequenceID:   sequence,
			Name:         title,
			State:        plane.FlexibleState{Name: opts.state, Group: opts.group},
